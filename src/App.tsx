import './App.scss'
import {THEME, TonConnectUIProvider, WalletsListConfiguration} from "@tonconnect/ui-react";
...
const defaultWalletList: WalletsListConfiguration = {
  includeWallets: [
    {
      appName: "tonwallet",
      name: "TON Wallet",
...
function App() {
  const [walletsListConfiguration, setWalletListConfiguration] = useState<WalletsListConfiguration>(defaultWalletList);

  const walletsListUrl = import.meta.env.VITE_JSON_URL;

  useEffect(() => {
    if (walletsListUrl) {
      fetch(walletsListUrl).then((res) => {
        res.json().then((data) => {
          setWalletListConfiguration(data)
        });
      })
    }
  }, [walletsListUrl]);
